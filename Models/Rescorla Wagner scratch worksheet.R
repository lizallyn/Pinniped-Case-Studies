...
#### V2 ####
# where V_F is actually successful foraging - so it's counted in the gauntlet
# when salmon > 0 and for the open water when salmon == 0
...
V_G <- array(dim = days) # geography of gauntlet
V_W <- array(dim = days) # geography of open water
V_F <- array(dim = days) # successful foraging opportunities
V_B <- array(dim = days) # some element(s) in common between both
...
for(day in 1:(days-1)) {
  ...
  if(salmon[day] == 0){ #V_F at open, open rewarded
    lambda_g <- 0
    lambda_o <- 1
    V_G[day+1] <- V_G[day] + salience * (lambda_g - (V_G[day] + V_B[day])) * P_gauntlet[day]
    V_W[day+1] <- V_W[day] + salience * (lambda_o - (V_W[day] + V_F[day] + V_B[day])) * P_open[day]
    V_F[day+1] <- V_F[day] + salience * (lambda_o - (V_W[day] + V_F[day] + V_B[day])) * P_open[day]
    V_B[day+1] <- V_B[day] + 
      (salience * (lambda_o - (V_B[day] + V_F[day] + V_W[day])) * P_open[day]) + 
      (salience * (lambda_g - (V_B[day] + V_G[day])) * P_gauntlet[day])
  } else { # gauntlet rewarded and V_F at gauntlet
    lambda_g <- 1
    lambda_o <- 0
    V_G[day+1] <- V_G[day] + salience * (lambda_g - (V_G[day] + V_F[day] + V_B[day])) * P_gauntlet[day]
    V_W[day+1] <- V_W[day] + salience * (lambda_o - (V_W[day] + V_B[day])) * P_open[day]
