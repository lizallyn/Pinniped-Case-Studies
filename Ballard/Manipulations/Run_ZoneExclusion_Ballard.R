...
# parameters
zone_parameters <- data.frame(rbind(start_loop, end_loop, num_seals, 
...
# salmon
zone_salmon <- rbind(salmon_escapement, salmon_eaten, salmon_catch)
rownames(zone_salmon) <- c("Escapement", "Eaten", "Catch")

zone_salmon_vars <- data.frame("Day" = 1:days + (start_loop - 1),
...
zone_salmon_vars$Eaten_salmon <- zone_salmon_vars$Eaten_Sockeye + zone_salmon_vars$Eaten_Chinook + 
  zone_salmon_vars$Eaten_Coho

# pinnipeds
zone_pinniped_harvest <- c(Pv = sum(H), Zc = sum(H_zc), Ej = sum(H_ej))
