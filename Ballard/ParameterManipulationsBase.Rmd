...
salmon_escapement <- data.frame(Seals = num_seals, Ej = num_ej, Zc = num_zc, Sockeye = escape_sockeye[days], Chinook = escape_chinook[days], Coho = escape_coho[days])
...
Escape <- data.frame(Seals = 0, Ej = 0, Zc = 0, Sockeye = 0, Chinook = 0, Coho = 0)
for(i in 1:length(num_seals_list)){
  ...
  source("Ballard/04_initialize_variables.R")
  source("Ballard/assembleLegosBallard.R")
  Escape[i,] <- data.frame(Seals = num_seals, Ej = num_ej, Zc = num_zc, salmon_escapement)
}
